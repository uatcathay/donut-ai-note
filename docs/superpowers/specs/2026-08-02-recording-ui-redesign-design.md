...
**逐幀正規化**：各頻段的值不直接映射成高度，而是先除以「當下的峰值」再映射——講話大聲小聲都能撐滿整排，起伏最明顯。基準值由**會衰減的峰值追隨器**維持（`peak = max(frameMax, peak × 0.93)`）而非直接取當幀最大值，否則整排每幀都會劇烈重新縮放、看起來像在抽搐。靜音判定不能用固定門檻——每個環境的底噪不同，而正規化會把任何存在的訊號放大到滿格。因此改為**自動追蹤噪音底線**：`noiseFloor` 瞬間跟上更安靜的幀、只以每幀 0.2% 的速率適應變吵的環境，所以它追的是「這個房間最安靜時的音量」而不會被說話聲拉高；門檻取 `noiseFloor × 1.8`，並在正規化時把門檻減掉。模擬顯示安靜房間門檻自動落在 18、有風扇的房間落在 68，兩者在無人說話時都整排歸位，開始說話後都能撐滿 12%–100%。講完話後約 0.6 秒衰減回靜音狀態。模擬顯示同一幀內最高與最低長條的高度差由 30 個百分點增為 87 個。
