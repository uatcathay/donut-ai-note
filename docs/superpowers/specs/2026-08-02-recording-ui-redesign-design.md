...
2. 底噪以 EMA 追蹤其**平均 `noiseMean` 與波動幅度 `noiseDev`**，死區取 `noiseMean + noiseDev × 4`。只扣掉平均並不夠——底噪本身持續在平均值上下起伏，必須連波動一起讓過，安靜時整排才會真正靜止。
3. 底噪統計只在判定為安靜（`strength < 0.5`）時更新，否則說話聲會把底噪一起拉高。
