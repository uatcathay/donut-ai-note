...
- 標題槽為 `min-height: 36px` + `margin-bottom: 52px`（三個標題元素共用）。用 `min-height` 而非 `height`，長標題換行時區塊自然往下長，不會溢出壓到麥克風；固定的 52px 下邊距同時保證標題與麥克風之間永遠至少有這麼多空間。
- **已知取捨**：待機頁的 `<input>` 永遠單行，錄音頁的 `.rec-title` 會換行，因此**標題長到換行時，錄音頁的麥克風會比待機頁低**。這是為了「長標題不壓到麥克風」而刻意接受的位移；若日後改為單行加 `text-overflow: ellipsis`，即可回到完全零位移。
