...
- **本頁是唯一垂直置中的狀態**：`#view-processing` 以 `min-height: inherit` 取得 `.stage` 的高度並 `justify-content: center`，讓載入動畫與文字作為一組置中於 `.stage`。原先「距內容區頂端 80px」的規則已由此取代。因為 `.stage` 靠上固定，這個置中在任何視窗高度下都停在同一位置（在預設的 720 視窗中恰好等於視窗正中央）。
