...
**逐幀正規化**：各頻段的值不直接映射成高度，而是先除以「當下的峰值」再映射——講話大聲小聲都能撐滿整排，起伏最明顯。基準值由**會衰減的峰值追隨器**維持（`peak = max(frameMax, peak × 0.93)`）而非直接取當幀最大值，否則整排每幀都會劇烈重新縮放、看起來像在抽搐。峰值低於 `WAVE_GATE = 10` 時視為靜音，整排回到靜止高度，避免沒人講話時放大環境噪音；講完話後約 0.6 秒衰減回靜音狀態。模擬顯示同一幀內最高與最低長條的高度差由 30 個百分點增為 87 個。
