...
同事用「瀏覽器錄音 → 丟 NotebookLM 分析」做會議重點，但 NotebookLM 無公開 API、需手動操作。本工具要把整條流程自動化，並把結果寫進使用者自己的 Notion（NotebookLM 做不到），供個人日常使用。
...
- 不做會議中的講者辨識分軌（speaker diarization）為必要功能；若模型自然帶出講者換行則保留，但不特別實作。
...
**四個狀態**：
- **開始前**：標題輸入框（選填）+「開始錄音」鈕。
- **錄音中**：計時器（mm:ss）+ 音量波形 +「停止並分析」鈕。
- **處理中**：三步驟進度（上傳音檔 / Gemini 分析 / 寫入輸出）。
- **完成**：輸出連結（Notion URL 或 `.md` 路徑）+ 摘要與重點預覽 +「記錄新會議」鈕。

**錄音實作**：`navigator.mediaDevices.getUserMedia({ audio: true })` → `MediaRecorder`，收集 chunks 成一個 Blob；音量波形用 `AnalyserNode` 讀取即時音量畫在 canvas 上。
...
- 摘要 3-5 句；重點為精煉條列；逐字稿盡量完整、以講者換行分段（若可辨識）。
