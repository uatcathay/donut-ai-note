...
- **版面位置穩定**：切換狀態時中央大按鈕與其上方元素不得有任何垂直位移。`.stage` 固定 `min-height: 560px`（取自最高的完成頁）且 `justify-content: flex-start`；三個標題元素同為 `min-height: 36px` + `margin-bottom: 52px`（長標題自然往下長，不壓到麥克風；已知取捨是換行時錄音頁的麥克風會比待機頁低）；標題為空時 `.rec-title` 用 `visibility: hidden`（`.is-empty` class）保留空間，不得用 `display: none`；`#preview` 固定 `height: 400px` 且自身捲動，不得撐破 `.stage`。
